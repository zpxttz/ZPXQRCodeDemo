...
@interface ZPXWebView () <WKNavigationDelegate, WKUIDelegate>
/// wkWebView
...
- (void)webView:(WKWebView *)webView didStartProvisionalNavigation:(WKNavigation *)navigation {
    if (self.zpxWebDelegate && [self.zpxWebDelegate respondsToSelector:@selector(webViewDidStartLoad:)]) {
        [self.zpxWebDelegate webViewDidStartLoad:self];
    }
}
/// 当内容开始返回时调用
- (void)webView:(WKWebView *)webView didCommitNavigation:(WKNavigation *)navigation {
    if (self.zpxWebDelegate && [self.zpxWebDelegate respondsToSelector:@selector(webView:didCommitWithURL:)]) {
        [self.zpxWebDelegate webView:self didCommitWithURL:self.wkWebView.URL];
    }
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation {
    self.navigationItemTitle = self.wkWebView.title;
    if (self.zpxWebDelegate && [self.zpxWebDelegate respondsToSelector:@selector(webView:didFinishLoadWithURL:)]) {
        [self.zpxWebDelegate webView:self didFinishLoadWithURL:self.wkWebView.URL];
    }
...
- (void)webView:(WKWebView *)webView didFailNavigation:(WKNavigation *)navigation withError:(NSError *)error {
    if (self.zpxWebDelegate && [self.zpxWebDelegate respondsToSelector:@selector(webView:didFailLoadWithError:)]) {
        [self.zpxWebDelegate webView:self didFailLoadWithError:error];
    }
