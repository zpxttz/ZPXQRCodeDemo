...
+ (UIImage *)ZPXQRCode_generateWithDefaultQRCodeData:(NSString *)data imageViewWidth:(CGFloat)imageViewWidth {
    ...
    return [ZPXQRCodeTool createNonInterpolatedUIImageFormCIImage:outputImage withSize:imageViewWidth];
}
 ...
 *
 *  @param data    传入你要生成二维码的数据
 *  @param imageUrlString    image的网络地址
 *  @param logoScaleToSuperView    logo相对于父视图的缩放比（取值范围：0-1，0，代表不显示，1，代表与父视图大小相同）
 */
+ (UIImage *)ZPXQRCode_generateWithLogoQRCodeData:(NSString *)data imageUrlString:(NSString *)imageUrlString logoScaleToSuperView:(CGFloat)logoScaleToSuperView{
    ...
    UIImage *icon_image = [UIImage imageWithData:imageData];
    CGFloat icon_imageW = start_image.size.width * logoScaleToSuperView;
    CGFloat icon_imageH = start_image.size.height * logoScaleToSuperView;
    CGFloat icon_imageX = (start_image.size.width - icon_imageW) * 0.5;
    CGFloat icon_imageY = (start_image.size.height - icon_imageH) * 0.5;
...
+(void)scanningQRCodePermissionSuccess:(void(^)())success Failure:(void(^)(NSString *failStr))failure {
    ...
    if (device) {
        ...
        if (status == AVAuthorizationStatusNotDetermined) {
            ...
            if (failure) {
                failure(@"您拒绝了打开相机,请去-> [设置 - 隐私 - 相机 - app名称]");
            }
        } else if (status == AVAuthorizationStatusRestricted) {
            if (failure) {
                failure(@"因为系统原因, 无法访问相册");
            }
        }
    } else {
        if (failure) {
