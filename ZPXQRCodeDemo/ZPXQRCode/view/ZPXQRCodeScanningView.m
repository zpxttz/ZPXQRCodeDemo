...
/** 二维码冲击波动画时间 */
CGFloat const ZPXQRCodeScanningLineAnimation = 0.05;

@interface ZPXQRCodeScanningView ()
...
@implementation ZPXQRCodeScanningView
...
#pragma mark - - - 扫描边角imageView的创建
    ...
    UIImage *left_image = [UIImage imageNamed:@"ZPXQRCode.bundle/QRCodeLeftTop"];
    ...
    // 右上侧的image
    UIImage *right_image = [UIImage imageNamed:@"ZPXQRCode.bundle/QRCodeRightTop"];
    ...
    // 左下侧的image
    UIImage *left_image_down = [UIImage imageNamed:@"ZPXQRCode.bundle/QRCodeLeftBottom"];
    ...
    // 右下侧的image
    UIImage *right_image_down = [UIImage imageNamed:@"ZPXQRCode.bundle/QRCodeRightBottom"];
...
- (UIImageView *)scanningline {
    if (!_scanningline) {
        ...
        _scanningline.image = [UIImage imageNamed:@"ZPXQRCode.bundle/QRCodeScanningLine"];
...
- (void)addTimer {
    ...
    self.timer = [NSTimer scheduledTimerWithTimeInterval:ZPXQRCodeScanningLineAnimation target:self selector:@selector(timeAction) userInfo:nil repeats:YES];
...
- (void)timeAction {
    ...
    if (flag) {
        ...
        flag = NO;
        [UIView animateWithDuration:ZPXQRCodeScanningLineAnimation animations:^{
            frame.origin.y += 5;
